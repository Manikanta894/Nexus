...
export async function publishToPlatforms(job, integrations = {}) {
  ...
  for (const platform of job.selectedPlatforms || []) {
    ...
    if (!caption) { results[platform] = { ok: false, error: 'No caption generated' }; continue }
    try {
      if (platform === 'linkedin' && integrations.linkedin?.accessToken) {
        const { postToLinkedIn } = await import('./social-post.js')
        const r = await postToLinkedIn({ caption, imageUrl: job.imageUrl, personUrn: integrations.linkedin.personUrn, accessToken: integrations.linkedin.accessToken })
        results[platform] = r
      } else if (platform === 'facebook' && integrations.facebook?.accessToken) {
        const { postToFacebook } = await import('./social-post.js')
        results[platform] = await postToFacebook({ caption, imageUrl: job.imageUrl, pageId: integrations.facebook.pageId, accessToken: integrations.facebook.accessToken })
      } else if (platform === 'instagram' && integrations.instagram?.accessToken) {
        const { postToInstagram } = await import('./social-post.js')
        results[platform] = await postToInstagram({ caption, imageUrl: job.imageUrl, igUserId: integrations.instagram.igUserId, accessToken: integrations.instagram.accessToken })
      } else if (platform === 'threads' && integrations.threads?.accessToken) {
        const { postToThreads } = await import('./social-post.js')
        results[platform] = await postToThreads({ caption, imageUrl: job.imageUrl, userId: integrations.threads.userId, accessToken: integrations.threads.accessToken })
      } else {
        results[platform] = { ok: true, mode: 'demo', message: `Demo publish (no ${platform} token configured)` }
      }
    } catch (e) {
...
export function estimateReach(job) {
  ...
  return { reach: Math.round((q / 100) * 800), engagement: Math.round((q / 100) * 40) }
}
