...
// Minimal db adapter over the in-memory store so collection-based libs (pipeline, news-radar) work.
...
async function handle(request) {
  ...
  if (route === '/news/config' && method === 'GET') return json({ config: newsConfig() })
  if (route === '/news/config' && method === 'PUT') {
    const body = await request.json().catch(() => ({}))
  ...
  if (route === '/news/analytics' && method === 'GET') return json(newsAnalytics(db.news_opportunities))
  if (route === '/news/learning' && method === 'GET') return json(newsInsights(db.news_opportunities))
